...
def execute(benchmark_config):
    ...
    if is_emulated:
        measurement_fn = Emulator(dataset=benchmark_config['problem'], model='BayesNeuralNet').run
    else:
        measurement_fn = benchmark_config['olympus_problem_obj'].run                                           

    repeat_results = []
    for repeat in range(num_repeats):
        Logger.log(f'Commencing repeat {repeat+1}/{num_repeats} for problem id : {problem_id}', 'INFO')

        campaign = Campaign()
        campaign.set_param_space(param_space)

        planner = BoTorchPlanner(
            goal=benchmark_config['goal'],
            **benchmark_config['optimizer_args']
        ...
        params_results_dict = {
            param.name : campaign.observations.get_params(as_array=True)[:, param_ix] for param_ix, param in enumerate(param_space)
        }
        values_results_dict = {measurement_name: campaign.observations.get_values(as_array=True).flatten()}
        results_df = pd.DataFrame({**params_results_dict, **values_results_dict})
