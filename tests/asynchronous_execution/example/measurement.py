...
class Worker:
    ...
    def __init__(
        ...
        measurement_fn,
        dump_dir,
        measurement_time_range=[5.0, 30.0],
    ):
    ...
    def measure(self, params):
        ...
        with open(
            f"{self.dump_dir}worker_result_{self.worker_id}_{measurement_timestamp}.pkl",
            "wb",
        ) as f:
        ...
        with open(
            f"{self.dump_dir}worker_status_{self.worker_id}.pkl", "wb"
        ) as f:
...
class Workers:
    ...
    def get_avail_workers(self):
        ...
        for worker in self.workers:
            if worker.is_available:
                ...
                status_file = (
                    f"{self.dump_dir}worker_status_{worker.worker_id}.pkl"
                )
    ...
    def get_new_experiments(self, priority_queue_dict):
        ...
        return [
            priority_queue_dict[ix]
            for ix, k in enumerate(exp_ids)
            if not k in self.used_exp_ids
        ]
    ...
    def monitor_pickup(self):
        while True:
            ...
            if not os.path.exists(self.pickup_file):
                ...
                Logger.log(
                    f"Available workers : {[worker.worker_id for worker in avail_workers]}",
                    "INFO",
                )

                if len(avail_workers) == 0:
                    Logger.log(
                        "No available workers at this time...", "WARNING"
                    )
                else:
                    ...
                    if len(new_exps) == 0:
                        Logger.log(
                            "All queued experiments have been completed or assigned to workers...",
                            "WARNING",
                        )
                    else:
                        ...
                        while worker_ix < len(
                            ...
                            process = multiprocessing.Process(
                                target=worker.measure, args=(param_vec,)
                            )
