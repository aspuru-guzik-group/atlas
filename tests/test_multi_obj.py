...
@pytest.mark.parametrize(
    "acquisition_kind", MIXED_CAT_CONT["acquisition_kind"]
)
@pytest.mark.parametrize(
...
@pytest.mark.parametrize(
    "acquisition_kind", MIXED_DISC_CONT["acquisition_kind"]
)
@pytest.mark.parametrize(
...
@pytest.mark.parametrize(
    "acquisition_kind", MIXED_CAT_DISC["acquisition_kind"]
)
@pytest.mark.parametrize(
...
@pytest.mark.parametrize(
    "scalarizer_kind", MIXED_CAT_DISC_CONT["scalarizer_kind"]
)
@pytest.mark.parametrize(
    "acquisition_kind", MIXED_CAT_DISC_CONT["acquisition_kind"]
)
@pytest.mark.parametrize(
    "acquisition_optimizer", MIXED_CAT_DISC_CONT["acquisition_optimizer"]
)
@pytest.mark.parametrize(
    "num_init_design", MIXED_CAT_DISC_CONT["num_init_design"]
)
@pytest.mark.parametrize(
    "is_constrained", MIXED_CAT_DISC_CONT["is_constrained"]
)
...
def run_continuous(
    ...
    if is_constrained:
        known_constraints = [
            KnownConstraintsGenerator().get_constraint("continuous")
        ]
    else:
...
def run_discrete(
    ...
    if is_constrained:
        known_constraints = [
            KnownConstraintsGenerator().get_constraint("discrete")
        ]
    else:
        known_constraints = None
...
def run_categorical(
    ...
    if is_constrained:
        known_constraints = [
            KnownConstraintsGenerator().get_constraint("categorical")
        ]
    else:
        known_constraints = None
...
def run_mixed_cat_cont(
    ...
    if is_constrained:
        known_constraints = [
            KnownConstraintsGenerator().get_constraint("cat_cont")
        ]
    else:
        known_constraints = None
...
def run_mixed_disc_cont(
    ...
    if is_constrained:
        known_constraints = [
            KnownConstraintsGenerator().get_constraint("disc_cont")
        ]
    else:
        known_constraints = None
...
def run_mixed_cat_disc(
    ...
    if is_constrained:
        known_constraints = [
            KnownConstraintsGenerator().get_constraint("cat_disc")
        ]
    else:
        known_constraints = None
...
def run_mixed_cat_disc_cont(
    ...
    problem_gen = ProblemGenerator(
        problem_type="mixed_cat_disc_cont", is_moo=True
    )
    surface_callable, param_space = problem_gen.generate_instance()

    if is_constrained:
        known_constraints = [
            KnownConstraintsGenerator().get_constraint("cat_disc_cont")
        ]
    else:
        known_constraints = None
...
if __name__ == "__main__":
    ...
    run_mixed_cat_disc_cont(
        "random", 1, False, "Hypervolume", "pymoo", 5, True
    )
