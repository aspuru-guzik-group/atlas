...
def run_single_molecular_param(num_init_design, batch_size):
	...
	delaney = pd.read_csv('data/delaney.csv')
	smiles = list(delaney['SMILES'])[:200] # categorical param molecular options

	features = np.load('data/delaney_features_mfp.npz', mmap_mode='r')
	descriptors = features['values'][:200, :] # (1116, 2048)
	descriptors = list([list(desc) for desc in descriptors]) # convert to list of lists

	param_space = ParameterSpace()
	param_space.add(ParameterCategorical(
		name='molecule',
		options=smiles,
