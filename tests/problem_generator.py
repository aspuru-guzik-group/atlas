...
class ProblemGenerator:
    def __init__(self, problem_type: str, use_descriptors=False, is_moo=False):
        ...
        self.accepted_problem_types = [
            "continuous",
            "discrete",
            "categorical",
            "contrained_continuous",
            "contrained_discrete",
            "contrained_cat",
        ]
    ...
    def add_descriptors(self, cat_surface):
        if self.use_descriptors:
            for param in cat_surface.param_space:
                desc = [
                    [float(i), float(i)] for i in range(len(param.options))
                ]
    ...
    def generate_instance(self):
        if self.problem_type == "continuous":
            if self.is_moo:
                # multi-objective
                surface_choice = str(
                    np.random.choice(self.allowed_cont_moo_surfaces, size=None)
                )
            ...
            surface_choice = str(
                np.random.choice(self.allowed_cat_surfaces, size=None)
            )
            surface_callable = Surface(
                kind=surface_choice, num_opts=NUM_CAT_OPTS
            )
            surface_callable = self.add_descriptors(surface_callable)
            return surface_callable, surface_callable.param_space

        elif self.problem_type == "mixed_cat_cont":
            surface_choice_cat = str(
                np.random.choice(self.allowed_cat_surfaces, size=None)
            )
            surface_choice_cont = str(
                np.random.choice(self.allowed_cont_surfaces, size=None)
            )
            surface_callable_cat = Surface(
                kind=surface_choice_cat, num_opts=NUM_CAT_OPTS
            )
            ...
            hybrid_surface_callable = HybridSurface(
                surfaces=[
                    surface_callable_cat,
                    surface_callable_disc,
                    surface_callable_cont,
                ],
                ...
                if surface_type == "disc":
                    ...
                    self.param_space.add(
                        ParameterDiscrete(
                            ...
                            low=0.0,
                            high=1.0,
                            options=list(np.linspace(0.0, 1.0, NUM_DISC_OPTS)),
                        )
                    )
                else:
