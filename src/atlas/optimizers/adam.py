...
class AdamOptimizer:
    ...
    def grad(self, sample):
        ...
        for i in self.select_idx:
            perturb[i] += self.dx
            gradient = (
                self.acqf(sample_to_acqf + perturb)
                - self.acqf(sample_to_acqf - perturb)
            ) / (2.0 * self.dx)
    ...
    def compute_update(self, sample: torch.Tensor) -> torch.Tensor:
        ...
        eta_next = eta * (
            torch.sqrt(1.0 - torch.pow(self.beta_2, self.iterations))
            / (1.0 - torch.pow(self.beta_1, self.iterations))
        )
        ms_next = (self.beta_1 * self.ms) + (1.0 - self.beta_1) * grads

        vs_next = (self.beta_2 * self.vs) + (1.0 - self.beta_2) * torch.square(
            grads
        )

        sample_next = sample - eta_next * ms_next / (
            torch.sqrt(vs_next) + self.epsilon
        )
