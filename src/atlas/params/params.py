...
import numpy as np
import torch
from olympus.campaigns import Campaign, Observations, ParameterSpace
...
class Parameters:
    def __init__(
        ...
        if len(observations.get_params()) > 0:
            ...
            self.expanded_scaled = forward_normalize(
                self.expanded_raw, self._mins_x, self._maxs_x
            )

            # scale the indexed representation (only the continuous dims)
            disc_cat_indexed = self.indexed_raw[
                :, self.disc_dims + self.cat_dims
            ]
            indexed_masked = self.indexed_raw.copy()
            indexed_masked[:, self.disc_dims + self.cat_dims] = 1.0

            self.indexed_scaled = forward_normalize(
                indexed_masked,
                np.amin(indexed_masked, axis=0),
                np.amax(indexed_masked, axis=0),
            )
            self.indexed_scaled[
                :, self.disc_dims + self.cat_dims
            ] = disc_cat_indexed
    ...
    def general_mask(self):
        return [
            True if ix in self.general_dims else False
            for ix in range(len(self.param_space))
        ]

    @property
    def exp_general_mask(self):
        return [
            True if ix in self.exp_general_dims else False
            for ix in range(self.expanded_raw.shape[1])
        ]
    ...
    def functional_dims(self):
        return [
            ix
            for ix in range(len(self.param_space))
            if ix not in self.general_dims
        ]
    ...
    def exp_functional_dims(self):
        return [
            ix
            for ix in range(self.expanded_raw.shape[1])
            if ix not in self.exp_general_dims
        ]
    ...
    def cont_dims(self):
        return [
            ix
            for ix, param in enumerate(self.param_space)
    ...
    def cont_mask(self):
        return [
            True if ix in self.cont_dims else False
            for ix in range(len(self.param_space))
        ]
    ...
    def disc_mask(self):
        return [
            True if ix in self.disc_dims else False
            for ix in range(len(self.param_space))
        ]
    ...
    def cat_mask(self):
        return [
            True if ix in self.cat_dims else False
            for ix in range(len(self.param_space))
        ]
    ...
    def exp_cont_mask(self):
        return [
            True if ix in self.exp_cont_dims else False
            for ix in range(self.expanded_raw.shape[1])
        ]
    ...
    def exp_disc_mask(self):
        return [
            True if ix in self.exp_disc_dims else False
            for ix in range(self.expanded_raw.shape[1])
        ]
    ...
    def exp_cat_mask(self):
        return [
            True if ix in self.exp_cat_dims else False
            for ix in range(self.expanded_raw.shape[1])
        ]
    ...
    def set_multi_fidelity_param_attrs(self, fidelity_params: int) -> None:
        ...
        self.fidelity_params_mask = [
            True if ix == fidelity_params else False
            for ix in range(len(self.param_space))
        ]
        # overwrite
        self.bounds[:, fidelity_params] = torch.tensor([1.0, 0.0])
        self._mins_x[fidelity_params] = 0.0
        self._maxs_x[fidelity_params] = 1.0
        ...
        for sample_ix, sample in enumerate(self.olympus):
            ...
            for elem, param in zip(sample, self.param_space):
                if param.type == "continuous":
                    ...
                    exp.append(float(elem))
                    ind.append(param.options.index(float(elem)))
                    counter += 1
    ...
    def _get_expanded_dims(self) -> Tuple[Any]:
        ...
        for param_ix, param in enumerate(self.param_space):
            if param.type == "continuous":
                ...
                dim += len(dims)
        return (exp_cont_dims, exp_disc_dims, exp_cat_dims, exp_general_dims)
    ...
    def param_vectors_to_indexed(
        ...
        if return_scaled:
            ...
            _mins_x, _maxs_x = np.amin(indexed_masked, axis=0), np.amax(
                indexed_masked, axis=0
            )
        ...
        for sample in indexed:
            ...
            for elem, param in zip(sample, self.param_space):
                if param.type == "continuous":
                    ...
                    expand.extend(
                        cat_param_to_feat(param, option, self.has_descriptors)
                    )
            expanded.append(expand)
...
if __name__ == "__main__":
    ...
    samples = [
        ...
        ParameterVector().from_dict(
            {"param_0": "x2", "param_1": 0.75, "param_2": 9.8, "param_3": -2.0}
        ),
        ParameterVector().from_dict(
    ...
    test_expanded = np.array(
        [
            [0.0, 0.0, 1.0, 0.0, 7.3, -1.9],
            [1.0, 0.0, 0.0, 0.5, 6.4, -1.0],
        ]
    )

    print("\n param_vectors_to_expanded")
    # param vectors to expanded
    res = params.param_vectors_to_expanded(
        test_param_vector, return_scaled=False
    )
    print(res)

    res = params.param_vectors_to_expanded(
        test_param_vector, return_scaled=True
    )
    print(res)

    print("\n param_vectors_to_indexed")
    # param vectors to indexed
    res = params.param_vectors_to_indexed(
        test_param_vector, return_scaled=False
    )
    print(res)
